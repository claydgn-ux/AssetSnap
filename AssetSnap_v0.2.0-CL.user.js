// ==UserScript==
// @name         AssetSnap · v0.2.0-CL
// @namespace    https://cl2.local/assetsnap
// @version      0.2.0
// @description  AssetSnap: D + Click or compact continuous mode to capture webpage images, preview quality, optionally convert format, and save with timestamp names.
// @author       CL
// @match        http://*/*
// @match        https://*/*
// @grant        GM_download
...
/*
 * AssetSnap_v0.2.0-CL
 * BASE VERSION: AssetSnap_v0.1.0-CL.user.js
 * RELEASE: 2026-09-15
 *
 * v0.2.0 scope:
 * - Tampermonkey list shows the full release identity.
 * - Replace the large floating controls with one compact draggable capsule.
 * - Compact Apple-style settings popover; existing core controls are retained.
 * - Hover target feedback shows intended source quality, dimensions when known,
 *   and format conversion intent.
 * - Download success card shows a thumbnail, dimensions, byte size when known,
 *   final format, quality source and conversion note.
 * - Output format setting: original / JPG / PNG, persisted globally for AssetSnap.
 * - Conversion is best-effort. If conversion cannot be completed, the original
 *   download path is used instead: acquiring the image always has higher priority.
 * - Unified timestamp naming: AssetSnap#yyyy-MM-dd_HH-mm-ss.ext; multiple captures
 *   in the same second append _01, _02, ... before the real final extension.
 * - v0.1.0 settings migrate in memory and are written as settings_v2 on change.
 *
 * Explicitly not added in this slice:
 * - Site-specific adapters.
 * - User-editable naming templates.
...
(function () {
    ...
    var CONFIG = {
        ...
        VERSION: '0.2.0',
        OWNER: 'CL',
        SETTINGS_KEY: 'settings_v2',
        LEGACY_SETTINGS_KEYS: ['settings_v1'],
        DEFAULTS: {
            shortcutKey: 'd',
            quality: 'high',
    ...
    function loadSettings() {
        ...
        if (!/^[a-z0-9]$/i.test(String(merged.shortcutKey || ''))) {
            merged.shortcutKey = CONFIG.DEFAULTS.shortcutKey;
        }
        merged.shortcutKey = String(merged.shortcutKey).toLowerCase();
    ...
    function createUI() {
        ...
        wrap.innerHTML = '' +
            ...
            '<div class="panel" id="panel">' +
                ...
                '<div class="group">' +
                    '<div class="row"><div class="rowText"><div class="rowTitle">快捷键</div><div class="rowSub">按住后左键点击图片</div></div><button class="key" id="keyBtn" type="button">D + Click</button></div>' +
                ...
                '</div>' +
                '<div class="group">' +
                    '<div class="row"><div class="rowText"><div class="rowTitle">文件命名</div><div class="rowSub">扩展名跟随最终实际格式</div></div><div class="fixedValue">AssetSnap#yyyy-MM-dd_HH-mm-ss</div></div>' +
                ...
                '<div class="group">' +
                    ...
                    '<div class="row"><div class="rowText"><div class="rowTitle">输入框保护</div><div class="rowSub">输入文字时不触发快捷键</div></div><button class="switch" id="inputSwitch" type="button"></button></div>' +
                '</div>' +
        ...
        refs.keyBtn.addEventListener('click', function () {
            ...
            showToast('按一个字母或数字键；Esc 取消', 'info', true);
        });
    ...
    function renderSettings() {
        ui.keyBtn.textContent = state.captureShortcut ? '按键…' : String(settings.shortcutKey).toUpperCase() + ' + Click';
    ...
    function renderMode() {
        ...
        ui.modeText.textContent = state.continuous ? ('连续下载 · ' + state.downloadCount) : (String(settings.shortcutKey).toUpperCase() + ' + 点击');
    ...
    function onKeyDown(event) {
        if (state.captureShortcut) {
            ...
            if (!event.metaKey && !event.ctrlKey && !event.altKey && /^[a-z0-9]$/i.test(event.key)) {
                event.preventDefault();
                event.stopImmediatePropagation();
                settings.shortcutKey = event.key.toLowerCase();
                state.shortcutHeld = false;
                state.captureShortcut = false;
                saveSettings();
                renderSettings();
                renderMode();
                showToast('快捷键已改为 ' + event.key.toUpperCase() + ' + Click', 'info', true);
            }
            return;
        }
        if (event.key === 'Escape') {
        ...
        if (String(event.key || '').toLowerCase() === settings.shortcutKey) {
    ...
    function allocateDownloadBase() {
        ...
        return CONFIG.PRODUCT_ID + '#' + stamp + (state.namingIndex > 0 ? '_' + pad(state.namingIndex) : '');
    }
